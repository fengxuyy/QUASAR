...
def test_audit_execution_cleanup_on_simulated_timeout(mock_popen, mock_workspace):
    ...
    with patch('os.getpgid', return_value=999), \
        ...
        # Execute
        result = execute_python.invoke({"code": "print('timeout')", "check_in_after": 5})
...
def test_audit_execution_environment_isolation(mock_workspace):
    ...
    execute_python.invoke({"code": "import os; os.environ['AUDIT_TEST_VAR'] = 'leaked'", "check_in_after": 5})
    ...
    result = execute_python.invoke({"code": "import os; print(os.environ.get('AUDIT_TEST_VAR', 'clean'))", "check_in_after": 5})
